  ...
  struct RdtscTransplanter : public ModulePass
  {
    ...
    bool runOnModule(Module &M)
    {
      ...
      for (Function &F : M.functions()) {
        // Check if this is a declaration and not a definition
        
            // Check for the "x86_64_sysvcc" attribute
            if (F.getCallingConv() == CallingConv::X86_64_SysV) {
                // Remove the calling convention
                F.setCallingConv(CallingConv::C);

                errs() << "Modified function: " << F.getName() << "\n";
            }
...
// 这边是对pass的注册
//  Register for opt
static RegisterPass<RdtscTransplanter> X("ChangeCC", "a pass to change the calling convention");

  ...
  typedef struct state
{
  ...
  struct StatTransplanter : public ModulePass {
    ...
    bool runOnModule(Module &M) {
                ...
                std::string xstatname="__xstat";
                oldfunc = M.getFunction(xstatname);
                ...
                for (Module::iterator mi = M.begin(); mi != M.end(); ++mi) {
                    ...
                    //不同函数的正则匹配规则
                    std::regex xstat_pattern("__xstat");

                    //待查找函数xstat
                    if (std::regex_match(fname,xstat_pattern)) {
                        find_flag=true;
                        //遍历该函数的users
                        for (User *U : f.users()) {
                            if (Instruction *Inst = dyn_cast<Instruction>(U)) 
                            {
                                ...
                                DataMorphingAndSync(Inst,xstatname);
                                errs() << "1" << "\n";
  ...
  //%14 = call i32 @__xstat(i32 %param0, i8* %10, %struct.stat* %12)
...
//这边是对pass的注册
// Register for opt
static RegisterPass<StatTransplanter> X("stat", "stat sync Pass");

...
typedef struct vprintf_configure_state
{
  ...
  struct VprintfFormatReinterpret : public ModulePass {
    ...
    bool runOnModule(Module &M) {
                ...
                for (Module::iterator mi = M.begin(); mi != M.end(); ++mi) {
                    ...
                    // 创建一个集合来跟踪已经处理过的指令，防止重复处理
                    std::unordered_set<Instruction*> processedInstructions;
                    //查找待匹配函数
                    ...
                    if (std::regex_match(fname,extv_printf_pattern) || std::regex_match(fname,extv_scanf_pattern)){
                        ...
                        // 遍历该函数的 users
                        for (User *U : f.users()) {
                            // 第一步：如果 U 是指令，遍历其所在函数的所有基本块中的 load 指令
                            if (Instruction *Inst = dyn_cast<Instruction>(U)) {
                              errs() << "Inst: " << *Inst << "准备修改的指令\n";
                                refreshVprintfState(&VprintfState);
                                Function *parentFunc = Inst->getFunction();
                                findVarstart = false;
                                if (parentFunc) {
                                    // 遍历 parentFunc 的所有基本块
                                    for (BasicBlock &BB : *parentFunc) {
                                        for (Instruction &I : BB) {
                                            if (LoadInst *LI = dyn_cast<LoadInst>(&I)) {
                                                // 检查 LoadInst 的第一个操作数是否有名字，并且匹配 xmm_pattern
                                                if (LI->getOperand(0) && LI->getOperand(0)->hasName()) {
                                                    std::string operandName = LI->getOperand(0)->getName().str();
                                                    if (std::regex_match(operandName, xmm_pattern)) {
                                                        // 使用 changeVprintfState 进行判断
                                                        if (changeVprintfState(&VprintfState, operandName)) {
                                                            // 判断成功，设置 findVarstart 标志
                                                            findVarstart = true;
                                                            break; // 跳出 LoadInst 循环
                                                        }
                                                    }
                                                }
                                            }
                                        }
                                        if (findVarstart) break; // 跳出基本块循环
                                    }

                                    if (findVarstart) {
                                        // 找到了之后开始生成 IR，将 GPR 都插入一个地址中
                                        for (User *Up : parentFunc->users()) {
                                            if (Instruction *UpInst = dyn_cast<Instruction>(Up)) {
                                              if (processedInstructions.find(UpInst) == processedInstructions.end()) {
                                                InsertGPRBeforeFundvprintf(UpInst);
                                                processedInstructions.insert(UpInst);
                                              }
                                            }
                                        }
                                        // 继续处理下一个用户
                                 ...
                                 errs() << "进入第二步\n";
                                // 第二步：如果未找到，遍历 U 所在函数的 users 的所在函数
                                if (!findVarstart && parentFunc) {
                                    for (User *U2 : parentFunc->users()) {
                                      refreshVprintfState(&VprintfState);
                                        if (Instruction *Inst2 = dyn_cast<Instruction>(U2)) {
                                            Function *callerFunc = Inst2->getFunction();
                                            if (callerFunc) {
                                                findVarstart = false;

                                                // 遍历 callerFunc 的所有基本块
                                                for (BasicBlock &BB2 : *callerFunc) {
                                                    for (Instruction &I2 : BB2) {
                                                        if (LoadInst *LI2 = dyn_cast<LoadInst>(&I2)) {
                                                            // 检查 LoadInst 的第一个操作数是否有名字
                                                            if (LI2->getOperand(0) && LI2->getOperand(0)->hasName()) {
                                                                std::string operandName2 = LI2->getOperand(0)->getName().str();
                                                                if (changeVprintfState(&VprintfState, operandName2)) {
                                                                    // 判断成功，设置 findVarstart 标志
                                                                    findVarstart = true;
                                                                    break; // 跳出 LoadInst 循环
                                                                }
                                                ...
                                                if (findVarstart) {
                                                    // 找到了之后开始生成 IR，将 GPR 都插入一个地址中
                                                    for (User *Up2 : callerFunc->users()) {
                                                        if (Instruction *UpInst2 = dyn_cast<Instruction>(Up2)) {
                                                          if (processedInstructions.find(UpInst2) == processedInstructions.end()) {
                                                            InsertGPRBeforeFundvprintf(UpInst2);
                                                            processedInstructions.insert(UpInst2);
                                                          }
                                                        }
                                                    }
                                                    // 继续处理下一个用户
                                                    continue; // 可根据需要选择是否跳出 U2 循环
                                                }
                                            }
                                        }
                                    }
 ...
 void refreshVprintfState(VprintfConfigureState *VprintfState)
  ...
  void InsertGPRBeforeFundvprintf(llvm::Instruction *callInst) {
    ...
    for(XMMRegisterCount=6; XMMRegisterCount<13; XMMRegisterCount++)
    {

...
//生产内部定义函数的间接跳转的模板函数的地方。如@add_two_number 到@sub_401230_add_two_number
...
llvm::Function *GetLiftedToNativeExitPoint(const NativeFunction *cfg_func) {
  ...
  //这边是生产extern_printf到printf的模板函数的地方
...
//该函数后，sqrt的args中有double的声明，而cos没有
  ...
  //在遇到直接call extern——printf的外部调用时，会进入这个函数生产call extern_printf(args6 )
  if (FLAGS_explicit_args) {

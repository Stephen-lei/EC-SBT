...
static void CreateLandingPad(TranslationContext &ctx,
  ...
  if (is_catch) {
    ...
    args[0] = ir.CreateLoad(ctx.stack_ptr_var);
    args[1] = ir.CreateLoad(ctx.frame_ptr_var);
    args[2] = ir.CreateLoad(var_value);
#endif
...
static void LiftInstIntoFunction(TranslationContext &ctx,
                                 llvm::BasicBlock *block,
                                 const remill::IntrinsicTable &intrinsics) {
  if(ctx.inst.category==remill::Instruction::kCategoryIndirectFunctionCall)
  {
    if(ctx.inst.pc==4236995)
    {
      printf("停在init_des里\n");
    }
    if(ctx.inst.pc==4208598)
    {
      printf("停在cjpdg_main里\n");
    }
     printf("停在翻译ID前\n");
  }
...
static llvm::Function *LiftFunction(const NativeModule *cfg_module,
  ...
  //推测这边的work_list里的一个节点代表一个基本块，而两个uint64_t参数分别代表起始地址和结束地址，故这边sort保证反正从最小的地址开始

...
NativeModule *ReadProtoBuf(const std::string &file_name,
  ...
  LOG(ERROR) << "Lifting program " << cfg.name() << " via CFG protobuf in "
            << file_name;
  ...
  for (auto f = 0, max_f = cfg.funcs_size(); f < max_f; ++f) {
    ...
    LOG(ERROR) << "Found function " << func->name << " at " << std::hex
              << func->ea << std::dec;
    ...
    if (func->is_exported) {
      ...
      LOG(ERROR) << "Exported function " << func->name << " at " << std::hex
    ...
    //去mcsema的注册函数表里查有没有这个函数。这里的gModule是默认配置，在loadprotofub之前已经配置好了。
  ...
  for (auto f = 0, max_f = cfg.external_funcs_size(); f < max_f; ++f) {
    ...
    if (!cfg_extern_func->has_name()) {
      LOG(ERROR) << "Ignoring external function at " << std::hex << func_ea
    ...
    if (auto found_func = module->TryGetFunction(func_ea)) {
      if (!found_func->is_exported) {
        ...
        LOG(ERROR) << "Exported function " << found_func->name << " at "
        ...
        LOG(ERROR) << "Ignoring external function '" << found_func->name
    ...
    LOG(ERROR) << "Found external function " << func->name << " at " << std::hex
              << func->ea << std::dec;
    ...
    LOG(ERROR) << "Found external function " << func->name << " via " << std::hex
              << func->ea;
    ...
    if (extern_func_it != name_to_extern_func.end()) {
      ...
      LOG(ERROR) << "External function " << func->name << " at " << std::hex
  ...
  for (auto v = 0, max_v = cfg.external_vars_size(); v < max_v; ++v) {
    ...
    if (!cfg_extern_var->has_name()) {
      LOG(ERROR) << "Ignoring unnamed external variable at " << std::hex
                   << var_ea << std::dec;
      continue;
    }
    ...
    if (auto found_func = module->TryGetFunction(var_ea)) {
      if (!found_func->is_exported) {
        ...
        LOG(ERROR) << "Ignoring external variable '" << found_func->name
    ...
    LOG(ERROR) << "Found external variable " << var->name << " at " << std::hex
              << var->ea << std::dec;
    ...
    if (extern_var_it != name_to_extern_var.end()) {
      ...
      LOG(ERROR) << "External variable " << var->name << " at " << std::hex
  ...
  for (auto s = 0, max_s = cfg.segments_size(); s < max_s; ++s) {
    ...
    for (auto v = 0, max_v = cfg_segment->vars_size(); v < max_v; ++v) {
      ...
      if (auto found_var = module->TryGetVariable(var_ea)) {
        ...
        LOG(ERROR) << "Ignoring variable '" << cfg_var->name() << "' at "
        ...
        LOG(ERROR) << "Found variable " << var->name << " at " << std::hex
                  << var->ea << std::dec;
      }
    }

    LOG(ERROR) << "Found segment " << segment->name << " [" << std::hex
  ...
  for (auto &entry : module->ea_to_func) {
    ...
    if (auto ll_func = gModule->getFunction(func->name); ll_func) {
      ...
      if (!ll_func->hasNUsesOrMore(1)) {
        LOG(ERROR) << "Erasing " << ll_func->getName().str();

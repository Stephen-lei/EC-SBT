...
cc_path = "/usr/lib/llvm-11/bin/clang"
...
def file_exist(all_dirs, file):
  """ Check if file exist.
  """
  path_test="/usr/lib/gcc/x86_64-linux-gnu/9/../../../../include/c++/9/math.h"
  for dir in all_dirs:
    path = str(dir,'utf_8') + "/" + str(file,'utf_8')
...
def visit_func_decl(node):
  ...
  if node.kind == CursorKind.FUNCTION_DECL:
    fun_parm_list = defaultdict(list)
    ...
    if not is_blacklisted_func(func_name):
      ...
      if is_valid_type(func_type):
        for i in node.get_children():
          checkparameter(i,fun_parm_list)
        
        key=func_name+" "+node.type.spelling
        FUNCDECL_LIST[key].append([mangled_name,node.type.spelling,fun_parm_list])
      else:
...
def checkparameter(node,list):
  ...
  if node.kind == CursorKind.PARM_DECL:
    list[node.spelling].append([node.spelling,node.type.spelling])
...
def write_cc_file(hfile, outfile):
  ...
  with open(outfile, "w") as s:
    ...
    s.write("\n\n")
    s.write(cc_header1)
    #第一层循环，遍历FUNCDECL_LIST
    for key in FUNCDECL_LIST.keys():
      type_values = FUNCDECL_LIST[key]
      #找到对应函数的声明定义
      #第二层循环，遍历一个函数声明中的函数个数（这边设计有点问题的）
      for type in type_values:
        #type[0,1,2]指向[mangled_name,node.type.spelling,fun_parm_list]
        s.write(" {0} {1}(".format(get_function_pointer(type[1]) ,type[0]))
        #到此写完“int remove(”
        param_list=type[2]
        #第三层循环，遍历一个函数对应的参数表[node.spelling,node.type.spelling]
        #num=param_list.len()/2
        num=len(param_list)
        for parm in param_list.keys():
          param_values = param_list[parm]
          type = param_values[0]
          #找到一个参数的声明定义
          #type[0,1]指向[node.spelling,node.type.spelling]
          s.write(" {0} {1}".format(type[1],type[0]))
          #到此写"const char * __filename"
          if num > 1:
            s.write(" , ")
            num=num-1
        ...
        s.write(");\n")
        #到此一个函数写完了
        #改成根据函数识别 return ，不能只是单纯的void
    s.write(cc_header2)
    #到此写完了具体的函数参数定义。下面开始写attribute
    #第一层循环，遍历FUNCDECL_LIST
    for key in FUNCDECL_LIST.keys():
      type_values = FUNCDECL_LIST[key]
      #找到对应函数的声明定义
      ...
      #type[0,1,2]指向[mangled_name,node.type.spelling,fun_parm_list]
        s.write("   (void *) {0}, \n".format(type[0]))
        #到此写完“reinterpret_cast<double *>(cos)”
...
def write_library_file(hfile, outfile):
  ...
  try:
    ...
    test_file= "/home/zyl/btfinaltest-llvm11_debug/mcsema/mcsema/OS/Linux/X86/test.cpp"
    if ARCH_NAME.lower() == 'amd64'.lower():
      #tu = cc_index.parse(hfile, args=['-x', libc_type, '-m64'])
      tu = cc_index.parse(test_file,args=['-x', 'c++', '-m64'])
